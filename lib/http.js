...
export function route(handler, unavailableMessage = 'Serviciul nu este disponibil momentan. Răspunsurile tale sunt păstrate în acest browser. Încearcă din nou.') { return async (req, res) => {
  res.setHeader('Cache-Control', 'no-store');
  try { await handler(req, res); }
  catch (error) {
    if (!error.status) console.error('API failure:', error.message);
    res.status(error.status || 503).json({ error: error.status ? error.message : unavailableMessage, ...(error.code ? { code: error.code } : {}), ...(error.questionId ? { questionId: error.questionId } : {}) });
  }
}; }
