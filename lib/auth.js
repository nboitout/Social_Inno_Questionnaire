...
export function authConfigurationError() {
  if (!process.env.ADMIN_PASSWORD) return 'ADMIN_PASSWORD is missing in this deployment. Set it in Vercel for this environment, then redeploy.';
  if (process.env.ADMIN_PASSWORD.length < 12) return 'ADMIN_PASSWORD must contain at least 12 characters. Update it in Vercel, then redeploy.';
  if (!process.env.SESSION_SECRET) return 'SESSION_SECRET is missing in this deployment. Set a separate random secret of at least 32 characters in Vercel, then redeploy.';
  if (process.env.SESSION_SECRET.length < 32) return 'SESSION_SECRET must contain at least 32 characters. It is separate from your admin password. Update it in Vercel, then redeploy.';
  return '';
}
export function authConfigured() { return !authConfigurationError(); }
