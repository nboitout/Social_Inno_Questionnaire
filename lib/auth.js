...
export function authConfigured() { return (process.env.ADMIN_PASSWORD?.length || 0) >= 24 && (process.env.SESSION_SECRET?.length || 0) >= 32; }
...
export function requireAdmin(req) {
  if (!authConfigured()) throw new HttpError(503, 'Administrarea nu este configurată.');
  ...
  } catch { throw new HttpError(401, 'Autentificare necesară.'); }
}
