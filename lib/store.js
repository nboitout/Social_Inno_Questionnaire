...
export const responseHeaders = [...legacyHeaders, ...survey.questions.map(q=>q.id), 'response_language'];
...
export function validateHeaders(headers, tab, writing=false) {
  const required = tab==='Visits' ? visitHeaders : ['submission_id','submitted_at','survey_version','session_id','duration_seconds','answers_json', ...(writing ? [...survey.questions.map(q=>q.id),'response_language'] : [])];
