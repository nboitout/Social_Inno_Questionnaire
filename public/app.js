import { survey, getQuestions, localized, accessTypes, selectedValues, toggleSelection, requiredQuestion, answerError, formatAnswer, participantFields, participantError, mainQuestionCount, workshopTheme, pruneAnswers } from './survey-config.js';
...
let position=Number.isInteger(draft?.position)?Math.max(0,Math.min(getQuestions(answers).length-1,draft.position)):0;
...
function home(){
 main.innerHTML=`<section class="hero homepage-hero"><div class="hero-copy"><h1 tabindex="-1">${t('hero')}<br><span>${t('heroAccent')}</span></h1><p class="hero-lead"><strong>${t('intro')}</strong></p><p class="hero-purpose">${t('purposeBefore')} <strong>${t('purposeFocus')}</strong> ${t('purposeAfter')}</p><p class="hero-practical"><span>${t('time')}</span></p><div class="hero-actions"><button class="primary" id="start">${Object.keys(answers).length?t('resume'):t('start')} <span>→</span></button></div><p class="hero-reassurance">${t('noRight')}</p></div><div class="journey-art" aria-label="${t('journey')}"><div class="art-top"><span>${t('artTitle').replace('\n','<br>')}</span><span class="art-asterisk">✳</span></div>${survey.sections.map((s,i)=>`${i?'<div class="join-line"></div>':''}<div class="journey-node ${i===survey.sections.length-1?'final-node':''}"><span class="node-number">0${i+1}</span><div><small>${localized(s.description,language)}</small><strong>${localized(s.title,language)}</strong></div><span>↗</span></div>`).join('')}<div class="art-bottom"><span>${t('artNote')}</span><span>01 — ${String(survey.sections.length).padStart(2,'0')}</span></div></div></section><details class="privacy"><summary>${t('privacy')}</summary><p>${t('privacyBody')}</p><button class="text-button" id="clear-draft">${t('clear')}</button></details>`;
...
function question(){
 ...
 const total=getQuestions(answers).length;
 shell(`<div class="progress-head"><span>${t('question')} ${q.number} / ${mainQuestionCount}</span><span>${Math.round(position/total*100)}%</span></div><progress max="${total}" value="${position}" aria-label="${t('progress')}"></progress>${q.theme?`<div class="workshop-theme"><small>${t('proposedTheme')}</small><h2>${localized(workshopTheme.title,language)}</h2><strong>${localized(workshopTheme.subtitle,language)}</strong><p>${localized(workshopTheme.body,language)}</p></div>`:''}<div class="question-heading"><span class="eyebrow">${localized(survey.sections.find(s=>s.id===q.section).title,language)}</span><h1 tabindex="-1" id="question-title">${esc(q.label)}</h1><p id="question-help">${esc(q.helper||(q.type==='multi'?t('many'):q.type==='single'?t('one'):''))}</p></div>${q.examples?`<div class="question-examples"><strong>${t('examples')}</strong><ul>${q.examples.map(e=>`<li>${esc(localized(e,language))}</li>`).join('')}</ul></div>`:''}${q.datasetNotice?`<p class="dataset-notice">${t('datasetNotice')}</p>`:''}${optional&&q.optionalForNonUsers?`<div class="branch-notice">${t('nonuser')}</div>`:''}<form id="question-form" novalidate><fieldset aria-labelledby="question-title" aria-describedby="question-help">${renderFields(q)}</fieldset><p class="error" id="error" role="alert"></p><div class="question-actions"><button type="button" class="secondary" id="back">← ${t('back')}</button><span class="optional-label">${optional?t('optional'):t('required')}</span><button class="primary" type="submit">${position===getQuestions(answers).length-1?t('review'):t('next')} →</button></div>${optional?`<button type="button" id="skip-question" class="text-button">${t('skip')}</button>`:''}</form>`);
...
function review(){
 ...
 document.querySelector('#back').onclick=()=>{position=getQuestions(answers).length-1;page='question';render();focusTitle();};
